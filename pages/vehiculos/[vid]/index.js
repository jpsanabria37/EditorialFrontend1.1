...
function VehiculoDetail({ vehiculo = {}, error }) {
  ...
  async function handleDelete() {
    ...
    if (confirmed) {
      setIsLoading(true);
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_BACKEND_URL}/api/v1/Vehiculo/${vehiculo.Id}`,
        {
          method: "DELETE",
        }
      );
      if (res.ok) {
        router.push(`/clientes/${vehiculo.Cliente.Id}`);
      } else {
        setIsLoading(false);
    ...
    <>
      <Dashboard>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
          <h1 className="text-3xl font-bold mb-4">{vehiculo.Marca}</h1>
          <div className="grid grid-cols-3 gap-4">
            ...
            <div>
              <p className="font-bold mb-1">Numero de Placa:</p>
              <p>{vehiculo?.NumeroPlaca}</p>
            </div>
            <div>
              <p className="font-bold mb-1">Numero de Motor:</p>
              <p>{vehiculo?.NumeroMotor}</p>
            </div>
          </div>

          <div className="mt-6">
            <BotonLink
              href={`${vehiculo?.Id}/actualizar`}
              className="mr-4"
